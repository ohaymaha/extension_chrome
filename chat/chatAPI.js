...
function timeout_init() {
    timeout = setTimeout('timeout_trigger()', 3600000);
}
/*----------------------------------------------------------------------------------------*
...
function deleteCookie(){
	var t = getCookie("__ohm__");
	setCookie("__ohm__", t, 0);
}
...
function CreateCookie(tokenKey, refreshKey, expiryTime) {
	...
	setCookie("r", refreshKey, 7);
	setCookie("__ohm__", tokenKey, expiryTime);
	setCookie("expiryTime", datetime, expiryTime);
}
function refreshtokenkey() {
    var refreshToken = getCookie("r");
    if (refreshToken == "" || refreshToken == "undefined") {
    	alert('refreshtokenkey');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (obj[0].State == 1) {
            	document.cookie ="r=" + obj[1].refreshKey + "; domain=.ohm.vn;path=/";
				document.cookie ="t=" + obj[1].tokenKey + "; domain=.ohm.vn;path=/";
            } else {
            	alert('refreshtokenkey ajax');
            	//window.location.replace("http://account.ohm.vn/login");
            }
        }
    });
...
function closeChannel(){
	if(getCookie("__ohm__")!=null){
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/closeChannel",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
			},
			type : "POST",
			dataType : "json",
			success : function(data) {
...
function createChannel(){
	if(getCookie("__ohm__")!=null){
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/createChannelAPI",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
...
function getListFriend(){
	if(getCookie("__ohm__")!=null){
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/getListUserFriend",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
				...
				if (data[0].State == 2) {
					...
					if(!data[1].length >0 && !data[2].length >0){
						$('#members-list').append("<li class='li-list-content-user0'>"
										...
										+"<img class = 'image-friend img-circle' src='images/hethong/ohg.png'>"
						...
						$.each(data[1],function(index,value){
							...
							$('#members-list').append("<li class='li-list-content-user"+value.userId+" stateonline"+value.onlineState+"'>"
									+"<span userName='"+value.userName+"' class='list-user-search' type='"+value.friendState+"' data='"+value.userId+"'  id='openBoxChat"+value.userId+"'onclick='createBoxChat(this)'>"
...
function getListUser(){
	if(getCookie("__ohm__")!=null){
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/listUser",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
...
//				$("#members-list-user li").remove();
				...
				if (data[0].State == 2) {
					...
					$.each(data[1],function(index,value){
						//tao cac li chua user online
						$('#members-list-user').append("<a href='#' class='on' userName='"+value.userName+"' id='openBoxChat"+value.userId+"' data='"+value.userId+"' type='"+value.friendState+"'  onclick='createBoxChat(this)'>"
					...
					$.each(data[2],function(index,value){
						//tao cac li chua user offline
						$('#members-list-user').append("<a href='#' class='off' userName='"+value.userName+"' id='openBoxChat"+value.userId+"' data='"+value.userId+"' type='"+value.friendState+"' onclick='createBoxChat(this)'>"
								+"<img class = 'image-friend' src='"+value.avatar+"'>"
								+"<span class='username'>"+value.userName+"</span>"
								+"</a>");
					});
...
function getListUserForIp(){
	if(getCookie("__ohm__")!=null){
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/getListUserForIp",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
			...
			success : function(data) {
				...
				if (data[0].State == 2) {
					...
					if(!data[1].length >0){
						...
						$.each(data[1],function(index,value){
							//tao cac li chua user online
							$('#members-list-user-for-ip').append("<li class='li-list-content-user"+value.userId+" stateonline"+value.onlineState+"'>"
									+"<span userName='"+value.userName+"' class='list-user-search' type='"+value.friendState+"' data='"+value.userId+"'  id='openBoxChat"+value.userId+"'onclick='createBoxChat(this)'>"
									+"<img class = 'image-friend img-circle' src='"+value.avatar+"'>"
									+"<span class='username'>"+value.userName+"</span>"
								+"</span>"
...
function getListUserSendStatusAddFriend(){
	if(getCookie("__ohm__")!=null){
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/getListUserSendStatusAddFriend",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
...
//					$("#members-list-user-send-add-friend span#members-list-user-send-add-friend"+id).remove();
					...
					$.each(data[1],function(index,value){
						//tao cac li chua user online
						$('#members-list-user-send-add-friend').append("<li class='li-list-content-user"+value.userId+" stateonline"+value.onlineState+"'>"
														+"<span userName='"+value.userName+"' class='list-user-search' type='"+value.friendState+"' data='"+value.userId+"'  id='openBoxChat"+value.userId+"'onclick='createBoxChat(this)'>"
															+"<img class = 'image-friend img-circle' src='"+value.avatar+"'>"
															+"<span class='username'>"+value.userName+"</span>"
														+"</span>"
...
function sendNotifyOn(){
	...
	if(getCookie("__ohm__")!=null){
		...
		obj.status = "notifyOn";
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/sendMessage",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
...
function addFriend(value){
	...
	if(getCookie("__ohm__")!=null){
		...
		obj.friendId = userId;
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/createAddFriend",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
...
//				alert("da gui thong bao ket ban");
				$("#boxchat"+userId+" .addFriend").remove();
...
function getListUserSendMessage(){
	if(getCookie("__ohm__")!=null){
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/getListUserHaveMessage",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
			...
			success : function(data) {
				if (data[0].State == 2) {
					...
					$.each(data[1],function(index,value){
						//tao cac li chua user online
						$('#content-list-link-message').append("<a href='#' id='openBoxChat"+value.userId+"' userName='"+value.userName+"' data='"+value.userId+"' type='"+value.friendState+"' onclick='createBoxChat(this)'>"
								+"<img class = 'image-friend' src='"+value.avatar+"'>"
								+"<span class='username'>"+value.userName
...
//																+"<div class='linechat-arrow'></div>"
																+"<div class='linechat-inner'>"+ replaceKeyCampaignOTA(message,userId)+"<p>"+getDateTime()+"</div>" 
															+ "</div>"
													+"</div>");
	...
	var a = "";
			if($("#boxchat"+userId).length==0){
				if(type=="1"){
		            ...
		            +"<span class='expand' onclick='slideUpOrDown(this)'><span class='close' data-toggle='tooltip' data-placement='top' title='an cua so'>&#x25BC;</span><span class='open' data-toggle='tooltip' data-placement='top' title='Mở cửa sổ'>&#x25B2;</span></span>"
		            ...
		            +"<h2><a href='#' title='go to profile'><img src='"+avatar+"' style='height:35px;' class='imgFriend' /></a> <span class='userName'>"+userName+"</span></h2>"
		            +"<div class='slider'>"
...
//			var getChatContentTime = "<span>"
//										+"<a href = '#' value='1' id='datetime1' onclick='getContentChat(this,"+value.id+")'>hom qua</a>"
//										+"<a href = '#' value='7' id='datetime17' onclick='getContentChat(this,"+value.id+")'>tuan truoc</a>"
//										+"<a href = '#' value='30' id='datetime130' onclick='getContentChat(this,"+value.id+")'>thang truoc</a>"
//										+"<a href = '#' value='360' id='datetime1360' onclick='getContentChat(this,"+value.id+")'>nam truoc</a>"
//									+"</span>"
//			$("#textBoxMessage"+value.id).append(getChatContentTime);
//			var userNameSender = value.text;
//			getContentChat(value.id,userNameSender);
			}else{
...
function onMessage(msg) {
	...
	if(status=="sendMessage"){
		var userNameTo = data[1].userName;
		...
		if(userIdTo!=userIdRecent){
			if($("#boxchat"+userIdTo).length==0){
				if(friendStatus=="1"){
		            ...
		            +"<h2><a href='#' title='go to profile'><img src='"+avatar+"' style='height:35px;' class='imgFriend' /></a> <span class='userName'>"+userNameTo+"</span></h2>"
		            +"<div class='slider'>"
		                    ...
		                    +"<div class='chat-input-text'>"
			                    ...
			                    +"<input placeholder='nhập tin nhắn' type='text' stateOnline='on' name='"+userIdTo+"' id='sendMessage"+userIdTo+"' class='' onkeypress='sendMessage(event,this)' />"			                    
...
//				var getChatContentTime = "<span>"
//											+"<a href = '#' value='1' id='datetime1' onclick='getContentChat(this,"+userIdTo+")'>hom qua</a>"
//											+"<a href = '#' value='7' id='datetime17' onclick='getContentChat(this,"+userIdTo+")'>tuan truoc</a>"
//											+"<a href = '#' value='30' id='datetime130' onclick='getContentChat(this,"+userIdTo+")'>thang truoc</a>"
//											+"<a href = '#' value='360' id='datetime1360' onclick='getContentChat(this,"+userIdTo+")'>nam truoc</a>"
//										+"</span>"
//				$("#textBoxMessage"+userIdTo).append(getChatContentTime);
				// bat su kien tooltip
				$('[data-toggle="tooltip"]').tooltip();
			}else{
		...
		$("#textBoxMessage"+userIdTo).append("<div class='block-line-chat left'>" 
													+"<div class='avatar-chatbox'>"
														+"<img class='img-circle' src='"+avatar+"'>"  
													+"</div>"
													+"<div class='linechat right'>"
//														+"<div class='linechat-arrow'></div>"
														+"<div class='linechat-inner'>"+ EmoticionString(message) +"<p>"+getDateTime()+" </div>" 
													+ "</div>"
											+"</div>");
		$("#boxchat"+userIdTo + " .logMsg").scrollTop(function () { return this.scrollHeight; });
	}
	if(status=="notifyOn"){
		...
		if($("#boxchat"+data[1].userId).length >0){
			...
			$("#boxchat"+data[1].userId+" input#sendMessage"+data[1].userId).attr("stateonline","on");
		}
//		alert(userNameSend + " da online");
		getListFriend();
//		getListUser();
		getListUserForIp();
	}
	if(status=="notifyOff"){
		// nếu friend mới đăng nhập thì notify cho list bạn bè biết (mới chỉ alert). đồng thời cập nhật lại bạn bè online, offline.
		var userNameSend = data[1].userName;
//		alert(userNameSend + " da offline");
		if($("#boxchat"+data[1].userId).length >0){
			$("#boxchat"+data[1].userId+" input#sendMessage"+data[1].userId).removeAttr("stateonline");
			$("#boxchat"+data[1].userId+" input#sendMessage"+data[1].userId).attr("stateonline","off");
		}
		getListFriend();
//		getListUser();
		getListUserForIp();
		...
		var userNameSend = data[1].userName;
//		alert(userNameSend + " gui yeu cau ket ban");
//		getListUser();
		getListUserForIp();
		getListUserSendStatusAddFriend();
...
function insertContentChat(userId2, chatContent, stateMessage){
	if(getCookie("__ohm__")!=null){
		...
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/insertContentChat",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
...
function getContentChat(value,userId2){
	if(getCookie("__ohm__")!=null){
		...
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/getChatContent",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
				...
				$.each(data[1], function(index, value){
					if(userId1 == value.to){
						$("#textBoxMessage"+userId2).append("<div class='block-line-chat right'>" 
																	...
																	+ "<div class = 'linechat left'>"
//																		+"<div class='linechat-arrow'></div>"
																		+"<div class='linechat-inner'>"+ value.content+"<p>"+ value.date+"</div>"
																	+ "</div>"
															+"</div>");
					}else{
...
function getContentChatByPage(userId2,page){
	if(getCookie("__ohm__")!=null){
		...
		obj.page = page;
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/getListChatContentByPage",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
...
//																			+"<div class='linechat-arrow'></div>"
							...
							avatar = $("#boxchat"+value.to+" img.imgFriend").attr("src");
...
function searchUser(value){
	...
	if(getCookie("__ohm__")!=null){
		...
		$.ajax({
			url : "http://chat.ohm.vn/chatAPI/getListUserForSearch",
			beforeSend : function(request) {
				request.withCredentials = true;
				request.setRequestHeader(
						"E8668OHM",getCookie("__ohm__"));
			...
			success : function(data) {
				if(data[0].State==1){
					...
					$.each(data[1],function(index,value){
								$("#content-list-link-search-friend").append("<li class='li-list-content-user"+value.userId+" stateonline"+value.onlineState+"'>"
										+"<span userName='"+value.userName+"' class='list-user-search' type='"+value.friendState+"' data='"+value.userId+"'  id='openBoxChat"+value.userId+"'onclick='createBoxChat(this)'>"
											+"<img class = 'image-friend img-circle' src='"+value.avatar+"'>"
											+"<span class='username'>"+value.userName+"</span>"
										+"</span>"
...
function replaceKeyCampaignOTA(value, userId){
...
var res="";
  ...
  var j=1;
	  $.each(result.keyword,function(value,index){
			resStr = "<div class='content-text-chat'>"+text + " </div>";
			var link ="";
			for(i=0; i<= index.length-1;i++){
				link +="<a href='"+index[i].url+"' ota='"+index[i].ota+"' campaign='"+index[i].name+"'>"+index[i].name
							+"<span class='dtohm'><img src='dtohm.png' title='ohm'></span>"
							+"<span class='plus-pay-ota'>+"+index[i].ota+"</span>"
						+"</a><br>";
			}
			url = "<span class='replace-string-campaign' id='replace-string-campaign"+j+"' data='"+j+"' onmouseover='displayCampaign(this)' onmouseout='hideCampaign(this)' >"
						+"<span style='color:red;'> "+value+" </span>"
						+"<span class='link-campaign-ota' id='link-campaign-ota"+j+"'>"+link+"</span>"
				 +"</span>";
			res = resStr.replace(" "+value+" ",url);
			j++;
	  });  
	  getReplaceKeyCampaignOTAStr(res,userId);
  }
})
}
function getReplaceKeyCampaignOTAStr(str, userId){
	$("#textBoxMessage"+userId+" .linechat-inner").append(str);
}