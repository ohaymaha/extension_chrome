...
$.fn.simpleColorPicker = function(options) {
    ...
    return this.each(function() {
        ...
        box.find('li.color-box').click(function() {
            if (txt.is('input')) {
              txt.val('');
              document.getElementById('color').style.background = opts.colors[this.id.substr(this.id.indexOf('-') + 1)];

                chrome.storage.sync.set({
                    OHMfavoriteColor: opts.colors[this.id.substr(this.id.indexOf('-') + 1)]
                }, function() {   
                });
